...
error = "My search didn't find any relevant results, try asking again with a different format."
...
def chat_completion_with_embeddings(
    ...
    """Performs chat completion using GPT, incorporating conversation history and embeddings."""
    if conversation_id not in conversation_history:
        conversation_history[conversation_id] = []

    # Create the query message using the dataframe
    query_msg = query_message(user_input, df, model=model)
...
def google(query: str, number: int, conversation_history: dict = None, conversation_id: str = "default"):
    ...
    service = build("customsearch", "v1", developerKey=GOOGLE.google_search_key)

    res = (
        service.cse()
        .list(
            q=query,
            cx=GOOGLE.google_search_engine_id,
            num=number,
        )
        .execute()
    )
