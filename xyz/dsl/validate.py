...
def validate_dsl(dsl: Any) -> tuple[bool, list[str]]:
    ...
    # Short-circuit on ``kind: "scripted"``.  Returning only this sentinel
    # is the actionable signal — burying it under a wall of schema errors
    # from a scripted-shaped document would obscure the real blocker
    # ("this kind isn't supported in v1") and waste the caller's time
    # chasing schema mismatches that are irrelevant once kind is fixed.
    if isinstance(dsl.get("kind"), str) and dsl["kind"] == "scripted":
        return False, [_SCRIPTED_ERR]

    # Collect every schema violation.  We want all errors (not first) so
    # the UI can show the full diff in one round-trip.
    errors: list[str] = [
        _format_error(err)
        for err in sorted(_VALIDATOR.iter_errors(dsl), key=lambda e: e.path)
    ]
